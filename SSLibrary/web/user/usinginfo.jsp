			...
			<tbody>
				...
				<c:forEach items="${booklist}" var="book" varStatus="status">
					<tr>
						...
						<c:if test="${user.isadmin != 's'}">
							<td><a class="btn btn-primary" href="javascript:;"
								onClick="if (confirm('연장하시겠습니까?')) location.href='userbookmodifyimpl.do?id=${book[0]}'">연장</a></td>
							<!-- 추후수정 -->
						</c:if>
						<c:if test="${user.isadmin == 's'}">
							<td></td>
					...
					<tr>
						...
						<td><a class="btn btn-primary" href="javascript:;"
							onClick="if (confirm('반납하시겠습니까?')) location.href='userbookremove.do?id=${book[0]}'">반납</a></td>
						<!-- 추후수정 -->
					</tr>
				</c:forEach>
				<tr>
					<td colspan="4"><a class="btn btn-primary btn-block"
						href="bookloglist.do">이력</a></td>
				</tr>
			</tbody>
		</table>
	</fieldset>
